...
*Tested today, two ways, both inconclusive — and the reason why is itself informative:*

1. Hooked `window.Shopify.analytics.publish` on a classic checkout, genuinely changed the shipping method. Zero calls captured.
2. Separately, on a **fresh** classic checkout (guest path, reached via the Shop Pay redirect + "Check out as guest" — see Finding 3), monitored raw network traffic for calls to the real GA4 endpoint (`analytics.google.com/g/collect?tid=G-QX88TBZC3G`, confirmed as the live property earlier via a product-page test that did show a `page_view` hit there). Watched through page load, a genuine shipping-method change, and a genuine payment-method change (Credit card → PayPal, no card data touched). **Zero `g/collect` requests fired for any of it** — not just for the shipping step, but for the payment-method change too.

That second result is the important one: we already know from the raw GA4 event data (Finding 6) that `add_payment_info` fires 7,976 times a year — it isn't broken. So a checkout session where even *that* produces no visible client-side collect call means this test isn't capturing the real dispatch channel at all, not that add_payment_info failed to fire this one time. The most likely explanation: checkout Customer Events (begin_checkout, add_shipping_info, add_payment_info, purchase) are dispatched server-side by Shopify — plausibly via the `monorail-edge.shopifysvc.com` endpoint, which fired repeatedly throughout this session — rather than through a client-side gtag call the way simple storefront events like `page_view` do. If that's right, no amount of browser-side network or JS instrumentation can answer this question; the events never touch the browser's network stack in an observable way regardless of whether they succeed.

*Remaining validation step:* the only methods left that can actually answer this are (a) GA4 DebugView, authenticated through Google's own reporting pipeline rather than browser-side observation, or (b) placing one real test order and checking GA4 Realtime a few minutes later for `add_shipping_info` — deliberately not done today to avoid a live transaction.
